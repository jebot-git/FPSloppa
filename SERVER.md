...
Experimental **CQ — CONQUEST** has a separate source-tree launcher and protocol, and its own `sv_cq_maxclients` (up to 64) and `sv_cq_bot_fill` settings. These do not raise `sv_maxclients` or the ordinary host limit. CQ cannot rotate into other modes or the lobby. `sv_cq_backend districts` opts into the experimental worker gateway; `sv_cq_worker_limit` (2–16) bounds its process pool. Monolithic remains the default. See [CQ setup, rules and current backend limits](docs/CONQUEST.md) before launching `./launch-conquest.sh --server`.
