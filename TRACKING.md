...
Body tracking animates VRM models and replicates to other players. In VR, valid body joints also show your own avatar from the shoulders down. The VRM first-person mesh hides the head/hair, replaces the controller gloves with avatar hands, and suppresses the duplicate avatar weapon. Local IK uses current tracking every frame; remote avatars keep their full meshes and smoothing. The movement and damage capsules stay aligned through bounded, collision-checked headset room-scale movement; body joints remain cosmetic. Capsule dimensions and controller-driven weapon aim stay unified. Missing joints fall back to the existing head/controller IK and animated, ground-adjusted feet. All clients and servers must use this release's `entryway-dm-10-melee` protocol.
...
Assign waist, chest, left/right foot, knee and elbow tracker roles in SteamVR. Use SteamVR as the active OpenXR runtime and enable its HTC Vive tracker extension support. Start the game, stand upright with feet pointing forward and arms relaxed, recenter, then select **CALIBRATE BODY** in the VR menu. Tracker roles bind to OpenXR’s standard `default_pose` action and Godot’s `default` pose; compatible bridges publishing `grip` or `tracker_pose` are also accepted. Each detected tracker gets a mounting offset to standing targets aligned with your current headset direction. Calibrate again after adjusting trackers or recentering. Tracker additions require recalibration. Partial sets work; hips plus two feet is a useful starting set.
...
The menu's **BODY TRACKING ON / OFF** disables all extra cosmetic tracking. Focus loss, stale OSC samples and unavailable native joints return to the existing IK. Your local body is visible during play only while at least one valid body joint is available; menus, death, focus loss, disabling tracking, or loss of body data restore the controller-glove view. Hand/finger data alone does not enable the body view. Native leg data is used only when the runtime supplies it; Quest upper-body support alone is not evidence of measured leg tracking.
