...
func setup(arena: Node) -> void:
	...
	vote=button(propose,"PROPOSE NEXT MATCH",func():
		if selector.ready_to_vote():game.lobby.submit(selector.modes.value,selector.maps.value,selector.loadouts.value))
	label(propose).text="Choose a mode, map and eligible loadout. A majority must approve the proposal."
...
func refresh() -> void:
	...
	selector.configure(options,game.match_mode.NAMES,game.armory.preferred)
	...
	next_match.text="NEXT: %s / %s / %s · %s\n%s"%[chosen.get("mode","?").to_upper(),chosen.get("map","?"),str(chosen.get("rules",game.armory.effective())).to_upper(),"VOTE APPROVED" if data.get("confirmed",false) else "ROTATION DEFAULT",data.get("result","")]
