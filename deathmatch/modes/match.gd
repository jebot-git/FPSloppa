...
func tick(delta: float) -> void:
	...
	if kind=="ctf":
		...
		for id in game.players:
			...
			if flags[own].dropped and nearby(id,flags[own].position,1.15):
			...
			if flags[enemy].carrier==id and flags[own].carrier==0 and not flags[own].dropped and nearby(id,bases[own],1.4):
		...
		if owner>=0:
			...
			while hill_credit>=1.0:
				...
				if game.intermission>0:return
func limit() -> int:return capture_limit if kind=="ctf" else hill_limit if kind=="koth" else game.frag_limit
...
func draw_objectives() -> void:
	...
	if visual_key!=key or not is_instance_valid(visuals):
		...
		if kind=="ctf" and bases.size()==2:
			for i in range(2):
				marker(bases[i],COLORS[i],TEAMS[i]+" BASE",1.4)
