...
static func positions(history: Array,now: float,rewind: float,current: Dictionary,heights: bool=false,yaws: bool=false) -> Dictionary:
	...
	for id in current:
		...
		if yaws:
			result[id]=lerp_angle(a.get("yaw",current[id].get("yaw",0.0)),b.get("yaw",current[id].get("yaw",0.0)),weight);continue
