...
func send(value: Dictionary) -> void:
	...
	if data.size()>LIMIT or output.size()+data.size()+4>LIMIT*2:failed=true;return
...
func poll() -> Array:
	...
	if count>0:
		var result:=peer.get_partial_data(mini(count,LIMIT+4-input.size()))
		if result[0]!=OK:failed=true;return messages
		received+=result[1].size();input.append_array(result[1])
	while input.size()>=4 and messages.size()<256:
