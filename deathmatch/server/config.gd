...
const DEFAULTS={"sv_hostname":"Entryway Arena","net_ip":"*","net_port":7777,"sv_maxclients":8,"fraglimit":20,"timelimit":10,"sv_log_level":"normal","sv_log_file":"","sv_log_max_mb":8,"sv_log_backups":3,"sv_voice":1,"sv_voice_backend":"builtin","sv_mumble_url":"","sv_votes":1,"map":"lqdm1","sv_maplist":"","sv_gametype":"dm","sv_gametypes":"","sv_friendlyfire":0,"capturelimit":5,"hilllimit":120}
const RANGES={"sv_log_max_mb":Vector2i(1,512),"sv_log_backups":Vector2i(1,9),"sv_friendlyfire":Vector2i(0,1),"capturelimit":Vector2i(1,100),"hilllimit":Vector2i(1,3600),"net_port":Vector2i(1024,65535),"sv_maxclients":Vector2i(1,MAX_CLIENTS),"fraglimit":Vector2i(1,100),"timelimit":Vector2i(1,60),"sv_votes":Vector2i(0,1),"sv_voice":Vector2i(0,1)}
...
static func parse(source: String) -> Dictionary:
	...
	for raw in source.split("\n"):
		...
		if RANGES.has(key):
			...
			if (value.is_empty() and not key in ["sv_maplist","sv_mumble_url","sv_gametypes","sv_log_file"]) or value.length()>(2048 if key=="sv_maplist" else 512 if key in ["sv_mumble_url","sv_log_file"] else 80): return {"error":"Line %d: empty or excessive value."%line_number}
			values[key]=value
	values.sv_gametype=str(values.sv_gametype).to_lower()
	if not values.sv_gametype in ["dm","tdm","ctf","koth"]:return {"error":"sv_gametype must be dm, tdm, ctf or koth."}
	...
	if modes.size()>4 or not Array(modes).all(func(mode):return mode in ["dm","tdm","ctf","koth"]) or not modes.has(values.sv_gametype):return {"error":"sv_gametypes must list up to four valid modes and include sv_gametype."}
	values["gametypes"]=[]
	for mode in modes:
