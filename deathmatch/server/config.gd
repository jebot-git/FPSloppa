...
static func parse(source: String) -> Dictionary:
	...
	if values.sv_cq_backend not in ["monolithic","districts"]:return {"error":"sv_cq_backend must be monolithic or districts."}
	if values.sv_cq_backend=="districts" and values.sv_gametype!="cq":return {"error":"District workers are exclusive to CQ."}
