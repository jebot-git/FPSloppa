...
const DEFAULTS={"sv_lobby":0,"sv_lobby_seconds":45,"sv_hostname":"FPSloppa","net_ip":"*","net_port":7777,"sv_maxclients":8,"fraglimit":20,"timelimit":10,"sv_log_level":"normal","sv_log_file":"","sv_log_max_mb":8,"sv_log_backups":3,"sv_voice":1,"sv_announcer":1,"sv_voice_backend":"builtin","sv_mumble_url":"","sv_votes":1,"sv_map_uploads":1,"map":"lqdm1","sv_maplist":"","sv_gametype":"dm","sv_gametypes":"","sv_friendlyfire":0,"capturelimit":5,"hilllimit":120,"dm_maplist":"","tdm_maplist":"","ctf_maplist":"","koth_maplist":"","ig_maplist":"","ft_maplist":"","cc_maplist":"","tf_maplist":"","as_maplist":""}
const RANGES={"sv_lobby":Vector2i(0,1),"sv_lobby_seconds":Vector2i(15,180),"sv_map_uploads":Vector2i(0,1),"sv_log_max_mb":Vector2i(1,512),"sv_log_backups":Vector2i(1,9),"sv_friendlyfire":Vector2i(0,1),"capturelimit":Vector2i(1,100),"hilllimit":Vector2i(1,3600),"net_port":Vector2i(1024,65535),"sv_maxclients":Vector2i(1,MAX_CLIENTS),"fraglimit":Vector2i(1,100),"timelimit":Vector2i(1,60),"sv_votes":Vector2i(0,1),"sv_announcer":Vector2i(0,1),"sv_voice":Vector2i(0,1)}
...
static func parse(source: String) -> Dictionary:
	...
	if not values.sv_gametype in ["dm","tdm","ctf","koth","ig","ft","cc","tf","as"]:return {"error":"sv_gametype must be dm, tdm, ctf, koth, ig, ft, cc, tf or as."}
	...
	if modes.size()>9 or not Array(modes).all(func(mode):return mode in ["dm","tdm","ctf","koth","ig","ft","cc","tf","as"]) or not modes.has(values.sv_gametype):return {"error":"sv_gametypes must list up to nine valid modes and include sv_gametype."}
	values["gametypes"]=[]
	for mode in modes:
