...
static func save_settings(values: Dictionary,path: String="") -> Error:
	...
	for key in defaults():cfg.set_value("presentation",key,values.get(key,defaults()[key]))
...
static func apply(game: Node,values: Dictionary) -> void:
	...
	if game.get_meta("texture_filter",-1)!=filtering:
		preload("res://deathmatch/maps/filtering.gd").new().apply(game,filtering,false)
