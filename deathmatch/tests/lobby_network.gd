...
func run() -> void:
	...
	if role=="server":
		g.dedicated=true;g.lobby.enabled=true;g.lobby.seconds=45;g.votes.allowed_modes=["dm","ig"];g.mode_maplists={"dm":["qsrc_dm1","qsrc_dm6"],"ig":["qsrc_dm6"]};g.map_rotation=["qsrc_dm1","qsrc_dm6"]
		...
		check(g.lobby.result()=={"mode":"ig","map":"qsrc_dm6","rules":"doom"},"Mode, map and required loadout tally agrees")
		g.lobby.until=g.clock
		check(await wait_for(func():return g.current_map=="qsrc_dm6" and g.players.size()==2 and not g.map_loading),"Both clients rejoin selected match")
		...
		g.lobby.submit("ig","qsrc_dm6")
		check(await wait_for(func():return not g.lobby.active() and g.current_map=="qsrc_dm6" and g.active and g.match_mode.kind=="ig"),"Client enters voted IG match")
