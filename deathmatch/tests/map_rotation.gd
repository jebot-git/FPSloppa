...
func run():
 ...
 if role=="server":
  var settings=Config.parse('set sv_maplist "qsrc_dm1 qsrc_dm6 lqdm4"')
  check(settings.values.maps==["qsrc_dm1","qsrc_dm6","lqdm4"],"Ordered map list parses")
  check(Config.parse('map lqdm7').values.maps==["lqdm7"],"Single map config remains supported")
  check(Config.parse('set sv_maplist "'+"qsrc_dm1 ".repeat(33)+'"').has("error"),"Excessive rotation list rejected")
  var config_path:="user://rotation_test_server.cfg"
  var file:=FileAccess.open(config_path,FileAccess.WRITE)
  file.store_string('set sv_gametype "ctf"\nset sv_gametypes "ctf koth"\nset net_port "28901"\nmap lqdm7\nset sv_maplist "qsrc_dm1 qsrc_dm6"\nset ctf_maplist "qsrc_dm1 qsrc_dm6"\nset koth_maplist "qsrc_dm1"\n');file.close()
  game._start_dedicated(PackedStringArray(["--config",config_path]))
  DirAccess.remove_absolute(config_path)
  check(game.active and game.current_map=="qsrc_dm1" and game.map_rotation==["qsrc_dm1","qsrc_dm6"],"Dedicated startup reads rotation and starts its first entry")
  ...
  for cycle in range(1,3):
   ...
   if cycle==1:
    ...
    check(game.votes.start(voter,"map","qsrc_dm6"),"Active player passes map vote; spectator excluded from electorate")
   else:game.votes.enabled=false;game._end_round();game.intermission=.1
   ...
   check(game.current_map==("qsrc_dm6" if cycle==1 else "qsrc_dm1"),"Rotation advances and wraps")
  ...
  game.votes.enabled=true;game.votes.cooldown=0
  var voter: int=game.players.keys().filter(func(id):return not game.players[id].spectator)[0]
  ...
  check(game.current_map=="qsrc_dm1" and game.match_mode.kind=="koth" and game.match_mode.scores==[0,0],"Mode vote changes rules with fresh objectives")
  ...
  if role=="second":
   var missing_hash:=FileAccess.get_sha256(game.Maps.Paths.folder("maps")+"qsrc_dm6.bsp")
   game.map_catalog=game.map_catalog.filter(func(row):return row.sha256!=missing_hash)
  for cycle in range(1,3):
   ...
   var expected_map: String="qsrc_dm6" if cycle==1 else "qsrc_dm1"
   var expected_hash:=FileAccess.get_sha256(game.Maps.Paths.folder("maps")+expected_map+".bsp")
   check(game.multiplayer.get_unique_id()==my_id and game.map_sha==expected_hash,"Connection ID retained and map bytes match, including downloaded aliases")
  ...
  check(game.current_map=="qsrc_dm1" and game.local_state().spectator==(role=="second"),"Mode vote preserves spectator role and current map")
