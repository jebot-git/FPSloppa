...
func run():
 ...
 if role=="server":
  ...
  file.store_string('set net_port "28901"\nmap lqdm7\nset sv_maplist "lqdm1 lqdm2"\n');file.close()
  ...
  for cycle in range(1,3):
   ...
   game._end_round();game.intermission=.1
   ...
   check(game.players.values().all(func(s):return s.kills==0 and s.hp==100 and s.owned==[2]),"New map resets score and inventory")
   await create_timer(.7).timeout
  await create_timer(1).timeout
 else:
  game.start_join(role,"127.0.0.1",28901)
