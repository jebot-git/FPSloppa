...
func _initialize() -> void:
	...
	while not stack.is_empty():
		...
		for file in DirAccess.get_files_at(folder):
			...
			if file.get_extension().to_lower() in ["bsp","vrm","pak","log"] or path.contains("AD-NOTICES") or file.begins_with("ad_arena_"):failures.append(path)
