...
func q(x: float,y: float,z: float) -> Vector3:return Vector3(-y*(1.25 if layout_test else 1.0),z,-x*(1.75 if layout_test else 1.0))/32.0+Vector3.UP*.05
...
func run() -> void:
	...
	game.selected_map="as_hislop" if layout_test else "as_hislop_tiny";game.start_host("Sludge escape",0,100,60,true,"as")
	...
	for hz in [60,72,90,120]:
		...
		for frame in hz*4:
			...
			if actor.position.x< (-6.25 if layout_test else -5.0):escaped=true;break
