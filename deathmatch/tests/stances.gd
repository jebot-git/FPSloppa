...
func run() -> void:
	...
	var idle_ready:=Gait.new()
	for i in 60:idle_ready.update(1./60.,Vector3.ZERO,"stand",true,body,true)
	idle_ready.update(1./60.,Vector3.FORWARD*9.4,"stand",true,body,true)
	check(idle_ready.assist_weight>.99 and idle_ready.offsets.left.length()>.01,"Already planted tracked feet animate on the first moving frame")
	body.left_foot.origin.z-=.1;idle_ready.update(1./60.,Vector3.FORWARD*9.4,"stand",true,body,true)
	check(idle_ready.assist_weight==0,"Immediate locomotion assist still yields to an intentional physical step")
	for i in 21:idle_ready.update(1./60.,Vector3.FORWARD*9.4,"stand",true,body,true)
	check(idle_ready.assist_weight>.99,"Assist resumes within 350ms after tracked feet settle")
