...
func run() -> void:
	...
	g.players[1].ammo[1]=0;check(not tf.can_fire(1,9),"TF sniper requires shotgun shells")
	g.players[1].ammo[1]=1;check(tf.can_fire(1,9),"TF sniper can fire with one shell")
