...
func setup(arena: Node) -> void:
	...
	if ResourceLoader.exists(cached):
		...
		var mesh:=new_mesh(game.current_map)
...
func team_rank(id: int) -> int:
	var members: Array=[]
	for other in game.players:
		if other<0 and not game.players[other].spectator and game.match_mode.same_team(id,other):members.append(other)
	members.sort()
...
func item_value(id: int,pickup: Dictionary) -> float:
	...
	match pickup.kind:
		"health","bonus":
			if not game.match_mode.fortress.enabled() and (pickup.kind=="bonus" or pickup.item==100):maximum=200
		...
		"armor":return maxf(0,int(pickup.get("amount",pickup.item*100))-s.armor)*.4
		"weapon":
	...
	return 0
func combat_upgrade(id: int,weapon: int) -> bool:
	# Slot numbers differ between profiles: UT Bio is 1 and its utility disc is 11.
...
func ammo_value(id: int,ammo: int) -> float:
	...
	if game.match_mode.fortress.enabled() and ammo==3 and s.tf_class in ["engineer","spy"]:usable=true
...
func mode_goals(id: int,brain: Dictionary,rows: Array) -> void:
	...
	if not mode.fortress.enabled():return
	...
	if mode.kind=="tb":return # TB uses actual universal dispensers, not implicit spawn resupply.
...
func travel_weight(kind: String,distance: float) -> float:
	...
	if kind=="tb_board" or (game.match_mode.kind=="tb" and kind=="guard"):return 1+minf(distance,45)*.02
	if kind=="equip":return 1+distance*.035
...
func plan(id: int,brain: Dictionary) -> void:
	...
	var rows: Array=[]
	var origin: Vector3=game.fighters[id].position
	if brain.goal_kind=="explore" and brain.enemy==0 and game.clock<brain.explore_until and origin.distance_to(brain.goal)>.5:return
	...
	var equip: bool=needs_equipment(id) and not game.match_mode.fixed_loadout() and not game.match_mode.fortress.enabled() and not game.match_mode.fortress.carrying(id)
	# An established hill holder keeps scoring; the screen can fetch supplies.
	...
	for index in mini(18,rows.size()):
		...
		if row.kind in ["cover","item","equip","supply","capture","heal"]:
	...
	if chosen.is_empty():
		var ceiling:Dictionary=navigation.ray(origin+Vector3.UP*.1,origin+Vector3.UP*1.7)
		if not ceiling.is_empty() and ceiling.normal.y<-.5:
			# A body below a low bridge cannot stand on the closest walking
			# polygon above it. Crawl clear before retrying that strategic route.
			var escape:Vector3=recovery_direction(origin,brain.goal-origin,id,brain)
			if not escape.is_zero_approx():
				brain.recover_prone=true;brain.recover_direction=escape;brain.recover_until=game.clock+.65
				brain.goal=origin+escape*1.2;brain.goal_key="crawl:escape";brain.goal_kind="explore";brain.hold=false;brain.support=0;brain.path=PackedVector3Array();brain.step=0;brain.explore_until=game.clock+2.5
				teamplay.count("crawl_escapes");return
		# A spawn can lie on a small disconnected nav island. Explore safe local
		# floor instead of treating "no route" as an intentional camping order.
		for index in 8:
			var angle:=TAU*float(posmod(index+id,8))/8
			var point:=origin+Vector3(cos(angle),0,sin(angle))*1.8
			var key: String="explore:%s"%Vector3i(round(point.x/2),round(point.y/2),round(point.z/2))
			if brain.avoid.has(key):continue
			var hit: Dictionary=navigation.ray(point+Vector3.UP*.5,point-Vector3.UP*.6)
			if hit.is_empty() or hit.normal.y<.7 or navigation.hazardous(hit.position):continue
			if not navigation.ray(origin+Vector3.UP,point+Vector3.UP).is_empty():continue
			brain.goal=hit.position;brain.goal_key=key;brain.goal_kind="explore";brain.hold=false;brain.support=0;brain.path=PackedVector3Array();brain.step=0
			teamplay.count("exploration_plans")
			brain.explore_until=game.clock+2.5
			return
...
func safe_shot(id: int,point: Vector3,explosive: bool=false) -> bool:
	...
	if not hit.is_empty():
		var pilot: int=game.match_mode.fortress.walkers.contact_pilot(hit)
		if pilot!=0 and game.match_mode.same_team(id,pilot):return false
...
func combat(id: int,brain: Dictionary,delta: float=.2) -> void:
	...
	if game.match_mode.fortress.enabled() and game.clock>=brain.action_at:
...
func steer(id: int,brain: Dictionary,delta: float) -> void:
	...
	if not brain.goal_key.is_empty() and brain.goal_kind in ["roam","search","explore"] and actor.position.distance_to(brain.goal)<(.5 if brain.goal_kind=="explore" else 1.5):
		brain.avoid[brain.goal_key]=game.clock+12;brain.plan_at=0
	...
	if brain.step<brain.path.size():
		while brain.step<brain.path.size()-1:
			var waypoint:Vector3=brain.path[brain.step]
			var reached:bool=actor.position.distance_to(waypoint)<.65
			# The first point is the nav projection of our current position, not
			# a ledge to climb. Swimmers' feet may lie below that surface too.
			if brain.step==0 or actor.in_water:
				reached=reached or Vector2(actor.position.x-waypoint.x,actor.position.z-waypoint.z).length()<.65 and absf(actor.position.y-waypoint.y)<2.5
	...
	# Stay below a low slab after the brief backing-out maneuver expires. A
	# swimmer under a bridge otherwise stands up into it every planning cycle.
	if brain.recover_prone or actor.collision_height<1.6:
		var ceiling:Dictionary=navigation.ray(actor.position+Vector3.UP*.1,actor.position+Vector3.UP*1.7)
		if not ceiling.is_empty() and ceiling.normal.y<-.5:
			var clearance:float=ceiling.position.y-actor.position.y
			s.prone=clearance<1.3;s.crouch=not s.prone;s.jump=false
		else:brain.recover_prone=false
