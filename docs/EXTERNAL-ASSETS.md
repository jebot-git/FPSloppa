...
The **Optional Community Map Pack** combines 53 maps across DM, TDM, CTF, KOTH, IG, FT and CC categories: 35 non-TF Arena Collection maps, all thirteen LibreQuake arenas and five reviewed community maps. TF maps and Tiny Assault layouts are excluded. Extract the pack separately, then run its `install.py --game-dir <external-asset-root>` or merge the selected `categories/<tag>/maps/` contents into your `maps/`. The installer preserves existing rotations and refuses changed-file conflicts. See [installation](../tools/optional_maps/PACK-README.md) and [categorized contents](../tools/optional_maps/CATALOG.md).
