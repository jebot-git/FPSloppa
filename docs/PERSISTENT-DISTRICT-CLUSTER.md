...
Default CQ is the fixed [81-district campaign](CAMPAIGN-81.md), capped at **128 total admitted players**, including waiting players. Transfers count once globally and reserve both districts locally. Each district retains its hard limit of 16. Generic non-CQ protocol experiments have capacity 16D, reaching 1,296 slots at eighty-one districts. Source rollback reservations and destination preparations both consume slots. Generic waiting joins consume a separate configurable allowance (default 128), not worker capacity. CQ waiting never adds places beyond its 128-player global cap. Already admitted campaign players can always enter reinforcement waiting after death. Full or draining destinations reject incoming transfer reservations. Fully occupied worlds therefore have no open destination gates.
...
Death remains resident until an explicit worker-validated respawn. Generic same-district respawn reuses its reserved slot; CQ releases it and searches all available allied districts before falling back to neutral outskirts or the inner city, otherwise entering the waiting room. Generic waiting deployment selects the nearest available district by graph distance, with no CQ team restrictions. A small global free-slot summary wakes gateways to retry connected waiting actors when deployment capacity changes, and clients may explicitly request deployment. A waiting actor that reconnects after gateway loss can resume and request deployment. The campaign desktop client renders an isolated waiting room for this state.
...
`init` defaults to the complete 81-district CQ campaign. Explicit `init --districts N` creates a generic non-CQ graph of 4–81 districts and provisions credentials for eighty-one stable identities and twenty-one gateways. This permits later activation without changing service credentials. Generic districts reuse the sixteen map templates. `init --campaign` is an explicit alias for the default and selects the complete, fixed 81-map campaign atlas; campaign membership cannot be edited into an incomplete layout, but its workers can be stopped or drained independently.
