...
- Without movement input, rise for **0.7 seconds**, hover until **1.5 seconds** after activation, then descend. Movement input during this lift, hover and descent allows gentle aerial adjustment up to **2 m/s**, accelerating at **3 m/s²**. Releasing movement brakes drift at **6 m/s²**; diagonal input does not increase the speed cap. With no movement input, the hover stays in place.
...
`deathmatch/tests/cq_jetpack.gd` covers activation, cooldown, stationary hover, restricted steering, high-ledge range, wall/ceiling collision, disabled states, dropped input edges, stale-life rejection, prediction and transferred trajectory state. All 44 checks pass. With engine physics configured at 30/60/120 Hz, unobstructed travel measured 32.49/32.77/32.78 m and apex 6.81/6.73/6.72 m. See [physics measurements](validation/cq-jetpack-physics.json).
