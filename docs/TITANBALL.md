...
| Second forward attackers | 222 m | Second checkpoint cleared |
| Defenders | 350 m, at their base | Entire round |

Forward bays sit 8 m behind their checkpoint and 15–17 m off the route centre, outside the robot's path. Coloured cover blocks direct fire from the approach while leaving lateral exits. There are six invulnerable universal engineer dispensers, usable by either team: one in the attacker hangar, one at the defender base, and two around each checkpoint (one on its approach side and one beyond it). The checkpoint stations sit near route distances 70/104 m and 215/246 m. These restore health, armour and ammunition at normal engineer-dispenser rates. There are no natural pickups or implicit proximity-to-spawn resupply in TB. A new round returns attackers to the initial group.

The ruined-city layout supplies low street cover and connected ground-floor ambush spaces, high crossings at 92 m, 179 m and 248 m with switchback ramp towers, and paired 4.2 m defensive positions at 286 m and 328 m. The middle overpass at 179 m connects directly to both 6 m balconies through two switchback flights per side. A gentle final-approach bend blocks the ground-level sightline from the second checkpoint to the base. The defender base has a heavy frontage and short reinforcement paths. This progression is a design intent. Autonomous bots board, escort, intercept and use resupply. Soldiers and demomen contest twenty-four authored firing positions on the overpasses, balconies and accessible fronts of the defensive platforms, using ordinary navigation and combat; exploratory 6v6 results are recorded in [the bot study](TITANBALL-BOT-BALANCE.md). Human competitive balance remains unmeasured.
...
Two visible gates sit at **80 m and 230 m** along the robot's route. Each grants **+3:00 once per round**, giving a maximum total budget of **16:00**. The server waits for a conservative 10 m rear/animated-footprint clearance: the robot's route origin must reach 90 m and 240 m respectively. Player crossings do not count. Pilot death, replacement, robot reset and repeated crossings cannot farm an extension. HUD progress and checkpoint awards replicate to clients; clients cannot grant time.

Only the middle checkpoint-to-checkpoint section has gained 50 m: the three legs are **80 / 150 / 120 m**, previously 80 / 100 / 120 m. Sharper S-bends around both checkpoints interrupt long sightlines. Cruise speed remains 0.8 m/s; the longer route, slower transitions and stationary reboarding delay consume more of the existing timer. See [the September balance test](TITANBALL-BALANCE-2026-09.md) for measured timing and autonomous 6v6 results.

An unearned extension cannot revive an expired round. Reaching and stopping at the route endpoint awards attacker victory. Expiration awards defender victory; neither result can be overwritten by a late objective event.
...
- Empty robots remain parked. Boarding accelerates to 0.8 m/s over five seconds. Dismounting, death or disconnect brakes to a halt over five seconds; the ladder redeploys after another three seconds fully stopped. Obstacles stop movement; a clear route permits restarting. An open route brakes at its endpoint.
- Distance is approximately **46 m in the first minute from rest**, then **48 m per minute** at cruise, assuming a clear route. Cruise speed is 2.88 km/h. The original 2.52 m stride now takes 3.15 s at cruise; distance-based playback preserves foot placement.
...
- Pilots have **200 HP maximum and no cockpit regeneration**. Living exit restores full normal class health and its maximum; death ejection does not revive the pilot. Medic passive regeneration remains a class ability.
- Occupied hulls accept only heavy ordnance: rockets, grenades, pipebombs, detpacks, Heavy primary fire, engineer sentries and Titan cannons. Direct hull contact is required; nearby splash and ordinary small arms cannot damage the pilot. This is fixed for TB, with no server toggle.
...
The [September balance changes](TITANBALL-BALANCE-2026-09.md) supersede historical timing and route measurements: ten-second voluntary-exit lock, five-second acceleration/braking, three-second stationary reboarding delay, and the middle-only 50 m extension.
