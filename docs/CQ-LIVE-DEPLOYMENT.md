...
Active profile data is durable in the control record. Offline profiles live in a separate SQLite `players` table or separate etcd keys, updated atomically with admission/logout. They do not enlarge every regional control document. Worker-owned counters checkpoint every five seconds and flush on death/respawn, logout and frozen handoff. A sudden worker loss can lose the latest uncheckpointed counter changes. Campaign resets preserve player profiles and lifetime counters. New identities enter the inner city and wait for an inner-city slot when it is full. Returning identities prefer their saved district and collision-checked location. If that district is full, offline, draining or homebase entry is locked, they choose the nearest friendly district, then eligible neutral territory, then the waiting room. Position/yaw checkpoint every five seconds and on orderly logout; health, ammunition, velocity and timers are not a persistent saved life. Death respawns retain allied-first/neutral-fallback rules. Transfer journals remain responsible for in-flight handoff recovery.
...
## Packaged quick scripts and moderation

The experimental server archive includes `prepare-deployment.sh`, `deploy-components.sh`, `start-master.sh`, `start-workers.sh`, `start-local-cq.sh` and `set-moderator-password.sh`. Worker hosts start their regional gateways and ENet facades automatically. Generated hosts also include a single `install.sh` for their assigned role, including the VPN ingress. `prepare-deployment.sh` defaults to the archive's `server/` assets; source checkouts should pass `--binary-dir Builds/CQModerator` or another completed campaign server build.

Set a moderator password in the private inventory before preparing deployment. Only master configurations receive its salted verifier. See [CQ-MODERATION.md](CQ-MODERATION.md) for F8 controls, F9 global announcements, rotation and capacity behavior. Allow approximately 44 Mbit/s extra application traffic at 128 listeners while a global announcement is active; media is gateway-to-gateway and does not enter the campaign database.
