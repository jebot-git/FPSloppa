...
var contrast:=OS.get_cmdline_user_args().has("--contrast")
var capture: String=OUT+("contrast/" if contrast else "")
var failures: Array=[]
var records: Array=[]
var world: Node3D
var camera: Camera3D
  ...
  if node is MeshInstance3D and node.mesh:
   for i in node.mesh.get_surface_count():
    ...
    if mat is ShaderMaterial and mat.get_shader_parameter("bake_texture") is Texture2D and not seen.has(mat):
     seen[mat]=true;mat.set_shader_parameter("liquid_warp",false);mat.set_shader_parameter("map_uv_offset",Vector2.ZERO);mat.set_shader_parameter("contrast_lighting",contrast);mat.set_shader_parameter("surface_variation",true)
...
func run() -> void:
 ...
 DirAccess.make_dir_recursive_absolute(capture+"render")
 ...
 # Minimal scaffold avoids starting gameplay/network/voice during map comparisons.
 ...
 var audit: Dictionary=JSON.parse_string(FileAccess.get_file_as_string("res://test-results/skyboxes/map-audit.json"));var sky_views: Dictionary={}
 for row in audit.maps:
  if Catalog.MAPS.has(row.id) and not row.views.is_empty():sky_views[row.id]=row.views[0]
 var maps: Array=JSON.parse_string(FileAccess.get_file_as_string(OUT+"build.json"))
 for row in maps:
  var map: String=row.map
  contents=Contents.new();check(contents.open("res://maps/"+map+".bsp"),map+": BSP contents")
  Atmosphere.apply(world,map)
  var env: Environment=world.get_node("Environment").environment
  check(not env.fog_enabled and not env.volumetric_fog_enabled,map+": no fog")
  if Catalog.MAPS.has(map):check(env.sky.sky_material is PanoramaSkyMaterial,map+": default panorama")
  var source: Node=ResourceLoader.load("res://maps/cache/"+map+"-lightmap1.scn","PackedScene",ResourceLoader.CACHE_MODE_IGNORE).instantiate();world.add_child(source)
  await physics_frame;await physics_frame
  var views: Array=[]
  if sky_views.has(map):
   var view: Dictionary=sky_views[map];views.append([vec(view.eye),vec(view.target),"sky"])
  else:views.append(spawn_view(source,false))
  if map=="tf_vesper":views.append([Vector3(15,2.2,24),Vector3(-8,12,-28),"nave"]);views.append(material_view(source,"stn_gr01_blu1","rose"))
  elif map=="tf_pressureworks":views.append([Vector3(16.25,1.7,23.75),Vector3(7,0,12),"floor"]);views.append(material_view(source,"tlight12","lamp"))
  elif map=="qsrc_dm2":views.append(material_view(source,"rune2_1","runes"))
  source.free();await process_frame
  for variant in ["before","raw","bc7","astc4-reference"]:
   var path: String=OUT+"original/"+map+".scn" if variant=="before" else OUT+"astc-reference/"+map+".scn" if variant=="astc4-reference" else "res://maps/cache/"+map+"-lightmap1"+("-bc7" if variant=="bc7" else "")+".scn"
   var level: Node=ResourceLoader.load(path,"PackedScene",ResourceLoader.CACHE_MODE_IGNORE).instantiate();world.add_child(level);still(level)
   for view in views:
    if view.is_empty():continue
    check(contents.at(view[0])==-1,map+" "+view[2]+": empty camera location")
    ...
    root.get_texture().get_image().save_png(capture+"render/"+map+"-"+view[2]+"-"+variant+".png")
    records.append({"map":map,"view":view[2],"variant":variant,"gpu_ms":stats(gpu),"cpu_ms":stats(cpu),"draws":stats(draws),"texture_bytes":stats(memory),"eye":str(view[0]),"target":str(view[1]),"fog":false,"panorama":Catalog.MAPS.has(map),"astc_decoded_reference":variant=="astc4-reference"})
   level.free();for i in 3:await process_frame
  var catalog_rows: Array=JSON.parse_string(FileAccess.get_file_as_string("res://deathmatch/maps/manifest.json"))
  var entry: Dictionary=catalog_rows.filter(func(item):return item.id==map)[0]
  var selected:=Loader.scene(entry)
  check(Loader.cache_matches(selected,row.bsp_sha256,"bc7"),map+": runtime selects native BC7 cache")
  selected=null
  print("STATIC_ASSETS_RENDER ",map)
  FileAccess.open(capture+"render.json",FileAccess.WRITE).store_string(JSON.stringify({"records":records,"failures":failures,"measured_frames":records.size()*90,"renderer":RenderingServer.get_current_rendering_method(),"gpu":RenderingServer.get_video_adapter_name(),"resolution":[1280,800],"msaa":4,"fog":false,"skyboxes_default":true,"contrast_lighting":contrast},"  "))
 world.free();contents=null;for i in 3:await process_frame
 print("STATIC_ASSETS_RENDER_RESULT ",failures);quit(0 if failures.is_empty() else 1)
