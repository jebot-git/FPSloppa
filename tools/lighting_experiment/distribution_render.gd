...
func run() -> void:
	...
	for input in prepared.maps:
		...
		var map: String=input.id
		var path: String=OUT+"off/"+map+".bsp";var hash:=FileAccess.get_sha256(path)
		...
		var low:=Loader.read(OUT+"low/"+map+".bsp")
	...
	var report:={"renderer":RenderingServer.get_current_rendering_method(),"gpu":RenderingServer.get_video_adapter_name(),"engine":Engine.get_version_info().string,"resolution":[1280,800],"msaa":4,"measured_frames":rows.size()*60,"records":rows,"failures":failures}
