...
class Metrics extends "res://deathmatch/server/log.gd":
 ...
 func record(event: String,data: Dictionary={},_detail: int=1) -> void:
  ...
  if event=="damage":
   var weapon: String=data.weapon;damage[weapon]=int(damage.get(weapon,0))+int(data.damage)
  if event=="match_event" and events.size()<250:events.append({"time":game.clock,"text":data.get("message","")})
...
func run() -> void:
 ...
 for id in game.players:
  if id>=0:continue
  game.players[id].team=abs(id)%2 if game.match_mode.team_game() else -1
  if options.mode=="tf":game.players[id].tf_next=classes[(abs(id)-1+int(options.get("class_offset",0)))%classes.size()]
  game._spawn(id)
