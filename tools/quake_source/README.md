...
Each BSP is compiled with full visibility and a four-sample-per-axis authored light bake. RGB lightmaps are embedded in the BSP so host downloads carry the lighting without sidecar requirements. The source retains its authored lights without added minimum light, sunlight, or bounce. A Quake-specific shader preserves dark samples without the generic brightness lift. Texture substitutions preserve special sky/liquid/clip semantics and use the explicit shared name-to-art dictionary, retaining protected texture semantics and button animation families. Makkon substitutions use the original donor names, dimensions and miptex bytes without resampling. Generic hash-selected substitutions have been removed. Switchable lights are baked on; key doors are unlocked for multiplayer. Buttons, shootable doors, secret doors, trigger chains and cyclic trains retain their authored activation. Original brushwork and multiplayer item/spawn locations are retained. Changes are recorded in `BUILD.json`; the game supplies its native pickups, weapon behavior, movers and trigger handling.
