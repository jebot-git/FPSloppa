...
const OUT="res://test-results/quake-restored/cache/"
var records: Array=[]
...
func run() -> void:
 ...
 for row in rows:
  if row.get("distribution","base")!="base" or (only.is_empty() and not str(row.id).begins_with("qsrc_dm")) or (not only.is_empty() and not row.id in only):continue
  ...
  var report: Dictionary={"map":row.id,"bsp_sha256":FileAccess.get_sha256(row.path),"geometry_preserved":true,"dark_faces":level.get_meta("baked_light_dark_faces",0),"quake_light_version":level.get_meta("quake_light_version",0),"packing":level.get_meta("lightmap_packing"),"fine_faces":level.get_meta("lightmap_fine_faces",0),"codecs":[]}
  if not str(row.id).begins_with("qsrc_dm"):
   level.free()
  else:
   ...
   var nav:=preload("res://deathmatch/bots.gd").new_mesh()
   var geometry_data:=NavigationMeshSourceGeometryData3D.new()
   NavigationServer3D.parse_source_geometry_data(nav,geometry_data,level)
   NavigationServer3D.bake_from_source_geometry_data(nav,geometry_data)
   assert(nav.get_polygon_count()>0)
   assert(ResourceSaver.save(nav,"res://maps/navigation/"+row.id+".res")==OK)
