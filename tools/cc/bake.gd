...
func run() -> void:
	...
	var rows: Array=JSON.parse_string(FileAccess.get_file_as_string("res://tools/cc/recipes.json"))
	for row in rows:
		...
		if id=="cc_hyperborea":mesh.set_meta("merge_rasterizer_cell_scale",.1)
		...
		print("CC_BAKED ",id," polygons=",mesh.get_polygon_count())
		level.free()
	quit()
