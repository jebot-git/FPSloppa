...
from titanball.build import City, VANTAGES
...
def main():
 ...
 old_lumps,old_extra=unpack(old);new_lumps,new_extra=unpack(new)
 ...
 for file in ['deathmatch/maps/manifest.json','maps/Ashfall/manifest.json','maps/Ashfall/texture-audit.json']:
  ...
  else:data.update(sha256=digest,bytes=len(new),source_sha256=hashlib.sha256(source.read_bytes()).hexdigest(),universal_resupply=6,tactical_vantages=VANTAGES)
 ...
 print(digest,f'6 stations, {VANTAGES} vantages; geometry/light/BSPX identical')
