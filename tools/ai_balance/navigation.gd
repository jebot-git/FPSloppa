  ...
  for random_seed in [7129,9137,12011]:
   for forced in [false,true]:
    ...
    if forced:
     ...
     brain.goal=goal;brain.goal_kind="roam";brain.goal_key="diagnostic";brain.path=game.bots.navigation.path(origin,goal);brain.step=0;brain.plan_at=game.clock+1000;brain.next=game.clock+1000
    var start:float=game.clock;var max_distance:=0.0;var trace:Array=[];var dry_seconds:=0.0
    while game.clock-start<30:
     ...
     if state.dead:break
    var result:Dictionary={"spot":name_here,"seed":random_seed,"forced_route":forced,"goal":goal,"seconds":game.clock-start,"final":actor.position,"max_displacement":max_distance,"dry_seconds":dry_seconds,"died":state.dead,"trace":trace}
