...
func run():
	...
	for zone in 16:
		...
		var build: Dictionary=JSON.parse_string(FileAccess.get_file_as_string(folder+"build.json"))
		check(visual.get_meta("baked_light_faces",0)>int(build.faces)*.8,"District geometry has baked lighting %d"%zone)
