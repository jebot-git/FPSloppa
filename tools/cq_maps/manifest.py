...
for zone in range(16):
 ...
 assert layout['revision']=='enclosed-city-3' and validation['covered_samples']/validation['roof_samples']>=.85
 assert not validation['route_failures'] and validation['carriageway_rays']>100 and validation['open_courtyards']==3 and validation['jetpack_clearance_samples']==42
 ...
 assert files['district.bsp']==build['sha256']==validation['bsp_sha256']
...
manifest=dict(profile='cq-district-bsp-1',revision=3,format='BSP29',grid=[4,4],district_width_metres=250,gate_width_metres=24,gate_height_metres=16,local_bounds_metres=[[-137,-3,-137],[137,112,137]],districts=rows)
