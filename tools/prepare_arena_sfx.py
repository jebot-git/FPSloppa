...
import array,hashlib,json,math,subprocess,wave,argparse
...
parser=argparse.ArgumentParser(description=__doc__);parser.add_argument("--jumps-only",action="store_true");args=parser.parse_args()
...
for i in range(3):
 foot=read(AUDIO/f'recorded/footstep_concrete_{i:03d}.ogg');grunt=read(ROOT/'tools/audio-sources/movement'/['3grunt1.wav','3grunt2.wav','3grunt6.wav'][i],1.40)
 save('jump_'+str(i),mix(.27,[(piece(foot,.10),0,.55),(piece(grunt,.16),.025,.65)]))
 if not args.jumps_only:save('land_'+str(i),mix(.25,[(piece(foot,.21),0,1),(piece(boot,.14),.055,.45)]))
if args.jumps_only:
 old=json.loads((OUT/'manifest.json').read_text());names={row['file'] for row in report}
 report=[row for row in old['files'] if row['file'] not in names]+report
...
print('Updated three distinct jump cues; other SFX preserved' if args.jumps_only else 'Authored %d mono SFX; peaks at or below -4 dBFS'%len(report))
