...
func walk_route(nav: RID,side: int,lane: int,extraction: bool=false) -> void:
 var start:=q(side*840,side*lane,8);var goal:=q(side*1872,side*-304,264)
 if extraction:start=q(side*1872,side*-304,264);goal=q(-side*1136,-side*368,8)
...
func run() -> void:
 ...
 report.flag_routes=[]
 for side in [-1,1]:
  var path_route:=NavigationServer3D.map_get_path(nav,q(1872*side,-304*side,260),q(-1136*side,-368*side,4),true)
 ...
 # The broken nave prevents a direct ground-level base-to-base shot.
 check(not clear_ray(q(-840,0,48),q(840,0,48)),"Broken nave interrupts direct sniper sightline")
 check(game.gates.filter(func(g):return g.get("elevator",false)).size()==4,"Four authored bell-tower elevators")
 ...
 # Actual turret targeting in a clear cloister lane: inside and outside 18 m.
 ...
 fortress.buildings[-9000]={"owner":-1,"team":0,"kind":"sentry","position":q(1280,1152),"ready":0.0,"next":0.0,"expires":game.clock+30,"hp":150,"map_owned":true}
 for item in [[q(1024,1152),true,"Sentry fires inside 18 m"],[q(640,1152),false,"Sentry respects 18 m range cutoff"],[q(1872,864),false,"Chapterhouse wall blocks sentry fire"]]:
  game.clock+=1;game.players[1].hp=100;game.players[1].armor=0;game.fighters[1].position=item[0]
  fortress.tick_sentries();check((game.players[1].hp<100)==item[1],item[2])
 ...
 for side in ([] if args.has("--no-walks") else [-1,1]):
  for lane in ([] if args.has("--extraction") else [-928,0,928]):await walk_route(nav,side,lane)
 ...
 for count in counts:
  ...
  print("VESPER_MATCH ",JSON.stringify(row))
