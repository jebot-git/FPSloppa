...
func run() -> void:
 var level=preload("res://deathmatch/maps/loader.gd").read("res://maps/tf_vesper.bsp")
 assert(level!=null)
 ...
 assert(ResourceSaver.save(mesh,"res://maps/navigation/tf_vesper.res")==OK)
 FileAccess.open("res://maps/VesperAbbey/navigation-sha256.txt",FileAccess.WRITE).store_string(FileAccess.get_sha256("res://maps/tf_vesper.bsp")+"\n")
 print("VESPER_NAV polygons=",mesh.get_polygon_count()," vertices=",vertices.size())
 level.free();quit()
