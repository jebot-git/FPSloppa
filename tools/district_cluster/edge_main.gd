...
func run() -> void:
	...
	if settings.backend[0]!="127.0.0.1" or settings.listen[0]!="127.0.0.1":quit(2);return
	var edge=load("res://deathmatch/server/cluster/edge.gd").new();edge.name="ClusterEdge";root.add_child(edge)
