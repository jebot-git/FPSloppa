...
assets_dir=root/'release-assets'/version
...
if release is None:
    release=api('POST',base,{'tag_name':version,'target_commitish':commit,'name':'FPSloppa '+version+' — Metal soundtrack, VR gestures and combat feedback','body':(assets_dir/'RELEASE-NOTES.md').read_text(),'draft':True,'prerelease':False})
...
for name in sorted(p.name for p in assets_dir.iterdir() if p.is_file()):
    ...
    connection=http.client.HTTPSConnection(url.netloc,timeout=180)
