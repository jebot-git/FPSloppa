"""Dedicated TF server plus eight clients, exercising all nine class abilities."""
from pathlib import Path
import argparse,json,os,subprocess,tempfile,time
ROOT=Path(__file__).resolve().parents[1]
def main():
    parser=argparse.ArgumentParser();parser.add_argument('bsp',type=Path);parser.add_argument('--demo',type=Path);parser.add_argument('--output',type=Path,default=ROOT/'test-results/tf-eight');args=parser.parse_args()
    if args.demo:
    ...
    with tempfile.TemporaryDirectory(prefix='fpsloppa-tf-eight-') as temp:
        try:
            for role in ['server']+[f'player{i}' for i in range(8)]:
                log=out/(role+'.log');handle=log.open('w');handles.append(handle)
                cmd=[os.environ.get('GODOT_BIN',str(Path.home()/'.local/bin/Godot_v4.7.2-stable_linux.x86_64')),'--headless','--xr-mode','off','--path',str(ROOT),'--script','res://deathmatch/tests/tf_eight.gd','--',role,str(args.bsp.resolve()),*([str(args.demo.resolve())] if args.demo else [])]
                ...
                if role=='server':
                    ...
                    if 'DM_HOST_READY' not in log.read_text():raise RuntimeError('TF host did not start: '+str(log))
