...
func run() -> void:
	...
	var rows: Array=JSON.parse_string(FileAccess.get_file_as_string("res://tools/community_maps/approved.json"))
	var maintained:=OS.get_cmdline_user_args().has("--maintained")
	if maintained:rows=JSON.parse_string(FileAccess.get_file_as_string("res://deathmatch/maps/manifest.json")).filter(func(row):return str(row.id).begins_with("lqdm"))
	for row in rows:
		...
		if maintained:
			print("MAINTAINED_CACHED ",id)
			level.free()
			continue # Texture-only changes preserve the existing navigation meshes.
		...
		print("COMMUNITY_BAKED ",id," polygons=",mesh.get_polygon_count())
		level.free()
	quit()
