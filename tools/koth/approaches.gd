...
func run():
 ...
 var rows=JSON.parse_string(FileAccess.get_file_as_string("res://tools/koth/recipes.json"));var result={}
 for row in rows:
  g._load_map(row.source)
  await physics_frame;await physics_frame
  var c=Vector3(-row.center[1],row.center[2],-row.center[0])/32
  var points=[]
  var reach=192 if row.source=="lqdm2" else 256
 ...
 FileAccess.open("res://test-results/koth/approaches.json",FileAccess.WRITE).store_string(JSON.stringify(result,"  "))
 g.free();quit()
