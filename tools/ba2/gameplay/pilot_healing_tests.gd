...
func run():
 ...
 for role in tf.CLASSES:
  ...
  check(g.players[1].hp==200 and tf.max_health(1)==200,"Occupied maximum is 200 and caps healing: "+role)
 ...
 for role in tf.CLASSES:
  ...
  row.speed=0.;row.to_speed=0.;g.clock+=2;g.fighters[1].position=w.transform(row)*w.LADDER
  await physics_frame;await physics_frame
  check(w.try_board(1,"test") and g.players[1].hp==200 and tf.max_health(1)==200,"Boarding grants 200 HP and maximum: "+role)
  g.players[1].hp=40;w.leave(1,true)
  check(g.players[1].hp==tf.definition(1).hp and tf.max_health(1)==tf.definition(1).hp and g.players[1].armor==35 and g.players[1].weapon==6,"Living exit restores full class health, armour and weapon: "+role)
 ...
 DirAccess.make_dir_recursive_absolute("res://test-results/titanball/pilot-healing")
 FileAccess.open("res://test-results/titanball/pilot-healing/unit.json",FileAccess.WRITE).store_string(JSON.stringify(result,"  "))
 print("PILOT_HEALING_RESULT ",result.passed)
