...
func run() -> void:
	...
	step(3);w.handle_player(1,false);w.handle_player(1,true)
	...
	check(r.speed>.1 and r.speed<.5,"Forced ejection preserves gradual robot braking")
	step(1);board();w.reset()
