...
func authority() -> void:
	...
	for i in 120:game.clock+=1./60.;w.tick(1./60.)
	...
	check(r.exit_lock>0 and not w.leave(pilot),"Authority blocks voluntary exit during the three-second boarding lock")
	for i in 60:game.clock+=1./60.;w.tick(1./60.)
...
func client() -> void:
	...
	check(game.match_mode.titanball.vantages.size()==12,"Authored high-ground positions replicate with the map state")
	...
	var forged: Dictionary=w.robots.test.duplicate();forged.distance=190.;tb.observe("test",forged)
