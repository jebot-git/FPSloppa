...
func run() -> void:
	...
	for role in tf.CLASSES:
		...
		check(w.robots.test.exit_lock==3. and w.snapshot()[0].exit_lock==3.,"%s starts a replicated three-second exit lock"%role)
	...
	s.hp=50;step(179)
	check(s.hp==50 and r.exit_lock>0. and not w.leave(1),"Damage persists and voluntary exit stays locked at 2.983 seconds")
	step(1);w.handle_player(1,true)
	check(w.mounted(1) and r.exit_lock==0.,"Held rejected jump does not auto-eject when three seconds expire")
	w.handle_player(1,false);w.handle_player(1,true)
	check(not w.mounted(1) and s.hp==50 and s.weapon==6 and s.armor==73 and s.tier==1,"Fresh jump after three seconds exits and restores only saved equipment")
	check(not w.try_board(1,"test"),"Existing reboarding cooldown remains enforced")
	prepare();w.try_board(1,"test");step(180)
	check(w.leave(1),"Use exit is allowed at exactly three seconds")
