...
func run() -> void:
 ...
 for item in [[Vector3(16.25,2.2,23.75),Vector3(0,5.6,-9.4),"turbine"],[Vector3(-15,1.88,-31.75),Vector3(0,2.5,-56),"pump-hall"],[Vector3(-7.5,1.7,-51.25),Vector3(0,1.25,-55.5),"flag-vault"],[Vector3(28.125,17.5,46.875),Vector3.ZERO,"overview"]]:
  camera.position=item[0];camera.look_at(item[1]);camera.make_current()
  await process_frame;await process_frame;await RenderingServer.frame_post_draw
  root.get_texture().get_image().save_png(out+"-"+item[2]+".png")
 for team in [0,1]:
  await shot(game,camera,game.ctf_spawns[team][0],"team%d-spawn"%team,out,false)
  await shot(game,camera,game.match_mode.bases[team],"team%d-flag"%team,out,true)
  await shot(game,camera,game.match_mode.captures[team],"team%d-capture"%team,out,false)
 game.free();quit()
