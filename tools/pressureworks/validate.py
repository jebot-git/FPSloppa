"""Run Pressureworks' in-engine objective, sentry, traversal and bot checks."""
from pathlib import Path
import concurrent.futures, hashlib, json, os, subprocess, tempfile, uuid

ROOT=Path(__file__).resolve().parents[2]
OUT=ROOT/'test-results/pressureworks'
FILES=['tools/pressureworks/acceptance.gd','tools/pressureworks/bake.gd',
       'deathmatch/arena.gd','deathmatch/bots.gd','deathmatch/fighter.gd',
       'deathmatch/modes/fortress.gd','deathmatch/modes/match.gd']
...
def main():
    OUT.mkdir(parents=True,exist_ok=True);bsp=ROOT/'maps/tf_pressureworks.bsp';sha=hashlib.sha256(bsp.read_bytes()).hexdigest()
    assert (ROOT/'maps/Pressureworks/navigation-sha256.txt').read_text().strip()==sha,'Navigation cache is stale; rebake'
    before=fingerprint();run_dir=OUT/('run-'+uuid.uuid4().hex[:10]);run_dir.mkdir()
    phases=[('static',['--static']),('6v6',['--no-walks','--players','12']),
            ('6v6-swapped',['--no-walks','--players','12','--swapped']),
            ('4v4',['--no-walks','--players','8']),('8v8',['--no-walks','--players','16'])]
    def phase(item):
        name,flags=item;target=run_dir/name
        with tempfile.TemporaryDirectory(prefix='pressureworks-') as temp, target.with_suffix('.log').open('w') as log:
            process=subprocess.run([os.environ.get('GODOT_BIN','godot'),'--headless','--xr-mode','off','--path',str(ROOT),'--script','res://tools/pressureworks/acceptance.gd','--',str(bsp),str(target),*flags],env=dict(os.environ,XDG_DATA_HOME=temp),stdout=log,stderr=subprocess.STDOUT,timeout=180)
        ...
        assert result['sha256']==sha and not result['failures'] and 'ERROR:' not in log,f'{name}: acceptance failures'
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
        results=dict(pool.map(phase,phases))
    assert fingerprint()==before,'Code changed during validation'
    report=results['static'];report['matches']=[]
    ...
    report['navigation_sha256']=hashlib.sha256((ROOT/'maps/navigation/tf_pressureworks.res').read_bytes()).hexdigest()
    ...
    (ROOT/'maps/Pressureworks/validation.json').write_text(json.dumps(report,indent=2)+'\n')
