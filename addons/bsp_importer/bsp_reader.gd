				...
				if (brush_plane.is_equal_approx(face_plane)):
					for brush_index in bspx_brush_stuff.planes_brush_map[brush_plane]:
						...
						if (brush.aabb.has_point(point_inside)):
							...
							if brush.collision_shape == null: continue
...
func create_collision_from_brushes(parent : Node3D, brushes : Array[BSPXBrush], parent_inv_transform : Transform3D):
	...
	for brush in brushes:
		...
		if (brush.planes.size() == 0): # If it's just an AABB, we can use a box.
			...
			if not convex_has_volume(convex_points):
...
# Degenerate brush intersections are not solid 3D hulls (Godot 4.7 rejects them).
func convex_has_volume(points: PackedVector3Array) -> bool:
	if points.size()<4: return false
	var a := points[0]
	var axis := Vector3.ZERO
	for p in points:
		if (p-a).length_squared()>axis.length_squared(): axis=p-a
	var normal := Vector3.ZERO
	for p in points:
		var n := axis.cross(p-a)
		if n.length_squared()>normal.length_squared(): normal=n
	if normal.length()<.00001: return false
