...
@export var collider_groups: Array # Unused, but this way we don't break script compatibility.
@export var collider_library: Array # Unused, intended to make inspecting easier
var spring_bones_cached: Array
...
func _ready() -> void:
	...
	for spring_bone in spring_bones:
		...
		var new_spring_bone: spring_bone_class.SpringBoneRuntimeState = spring_bone.create_runtime(skel)
