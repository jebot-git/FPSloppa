...
Exports and signature reports are under `test-results/`; APKs and SHA256 files are under `../Builds/Android/`. The builder verifies signatures and 16 KB ZIP alignment. Both exports retain plain GDScript and the VRM release-runtime compatibility patches. The Android builder embeds the verified base ZIP temporarily, removes its build input afterward, and keeps optional packs out of the APK. `--thin` explicitly produces a download-only development build; the normal verifier expects unified APKs.
...
The September renderer change removes OpenGL-specific avatar/decal workarounds and targets Vulkan on Quest/Pico. Export verification checks the actual packaged renderer settings. See the [renderer support assessment and Quest test status](docs/RENDERER-SUPPORT.md).
