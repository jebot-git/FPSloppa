...
const MAKKON_PACK="res://deathmatch/maps/texture_replacements/makkon-used.wad"
...
static func resolve(name: String) -> Dictionary:
	...
	var f:=FileAccess.open(MAKKON_PACK if row.get("pack","")=="makkon-used.wad" else PACK,FileAccess.READ)
	if not f:return {}
	...
	if row.has("makkon"):result.native_size=Vector2i(width,height)
	if has_glow:
