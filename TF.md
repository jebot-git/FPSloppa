...
|---|---:|---:|---|
| Scout | 75 / 25 | 125% | Axe, shotgun, nailgun. Three-second sprint; ten-second cooldown. |
| Sniper | 90 / 25 | 90% | TF sniper rifle, shotgun, axe. Sniper costs two nails and deals 90 before armor instead of instagib. Four-second focus raises it to 150 and halves movement; twelve-second cooldown. |
| Soldier | 150 / 100 | 85% | Axe, shotgun, super shotgun, rocket launcher. Aimed grenade with visible 1.2-second fuse; eight-second cooldown and one rocket cost. |
| Demoman | 120 / 75 | 95% | Axe, shotgun, grenade launcher. Throws one visible pipe grenade with gravity and world bounce; use again after 0.7 seconds of arming to detonate. One rocket; eight-second reuse cooldown. |
| Medic | 110 / 50 | 110% | Axe, shotgun, super shotgun, super nailgun. Aim at a teammate within six metres to heal 35 HP and extinguish; two-second cooldown. Regenerates three HP per second. |
| Heavy | 200 / 150 | 65% | Axe, shotgun, super shotgun, super nailgun. Four-second brace reduces incoming combat damage by 35% and halves movement; twelve-second cooldown. Hazards bypass brace. |
| Pyro | 125 / 75 | 100% | Axe, shotgun, rocket launcher and an eight-metre flamethrower using cells. Burning lasts three seconds. Aimed napalm grenade costs 20 cells with a ten-second cooldown. Fire resistance. |
| Spy | 90 / 25 | 105% | Axe, shotgun, super shotgun, nailgun. Disguise copies the nearest enemy’s validated VRM and identity, without invisibility. Dedicated servers may select cell-powered invisibility instead. Rear fist attacks deal at least 120 before armor. |
| Engineer | 100 / 75 | 100% | Axe, shotgun, super shotgun. Choose sentry or dispenser in the class menu. One of each, 60 cells apiece, three-second construction and 120-second lifetime. Aim at a damaged friendly building to repair 40 HP for ten cells. |
...
The distribution includes exactly two TF maps: **Pressureworks** (`tf_pressureworks`) and **Vesper Abbey** (`tf_vesper`). They are included in Base Assets and the TF rotation. The optional **Original-TF-Arenas** ZIP supplies the same two maps, navigation, editable sources, previews and notices for existing installations. Build it with `python3 tools/package_tf.py`; it verifies the compiled maps against their source, navigation, acceptance and texture receipts and writes the archive under `../Builds/`.

Select a map and TF in the host menu, or configure:

```text
set sv_gametype "tf"
set sv_gametypes "tf ctf dm"
set tf_maplist "tf_pressureworks tf_vesper"
set capturelimit 5
map tf_pressureworks
```

Both maps target 6v6 and support 4v4–8v8, with eight spawns per team, resupply and separate flag/capture positions. Pressureworks uses three entrances around a turbine yard; Vesper Abbey uses a gothic nave, upper sanctuaries, four elevators and permanent ramps. Their geometry and new tools are CC0; Makkon and LibreQuake textures retain their separate licences and provenance. Human competitive balance remains experimental.

LibreQuake maps and the older Ironspan/Relayworks arenas are excluded from the TF distribution selection. Existing custom BSPs can still be installed separately. Map and custom VRM transfers use the external `maps/` and `vrm/` directories.
...
Deliberate physical throws now receive up to **2.4× hand-speed assistance**, capped
at **26 m/s**. Gentle releases still drop the grenade. While holding ordnance, a
golden arc previews its current launch path through the first world collision (or
the timed fuse); pipes show up to two seconds. Swinging changes the preview and
throw distance. This is an estimate before bouncing or player impact, not a lock-on
or an extension of the fuse. The preview uses bounded updates and reuses its sweep
query; the server retains ammo, cooldown, obstruction and release validation.

In team modes, grabbing the offhand shoulder radio takes priority over the grenade
chord. Release the radio before arming ordnance. See [voice controls](VOICE.md).
...
## Pressureworks playtest map

`tf_pressureworks` is an original industrial TF map targeting 6v6 and supporting 4v4–8v8. Weathered brick pump stations, timber roofs and copper machinery flank a cobbled yard with grass and quarry rock, using 30 original Makkon and LibreQuake textures. It has three base entrances, walkable upper routes, eight spawns per team and separate flag/capture rooms. The existing 18 m sentry range reaches its tested defensive approaches. Source, licences, research and validation are in [maps/Pressureworks](maps/Pressureworks/README.md); rebuild and playtest instructions are included. The map is installed locally and appended to the TF maplist. Human competitive balance remains experimental.

## Vesper Abbey playtest map

`tf_vesper` is an original gothic TF map for 6v6, supporting 4v4–8v8. A ruined nave and lower cloisters lead to elevated flag sanctuaries, with four well6-style elevators and permanent ramp alternatives. Makkon Gothic Stone supplies the carved arches, rose windows and spires. [Source, previews, installation and validation](maps/VesperAbbey/README.md) accompany the locally installed map and TF rotation entry. Human competitive balance remains experimental.
